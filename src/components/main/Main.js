...
export default function Main(props) {
	const { isLoggedIn, isOpen, onArticleSave, showLessArray, articles, isPreloader, generateKey, q } = props;
		...
		<main className={`main ${isOpen? 'main__opened' : ''}`}>
			...
			<ul className={`main__list${isPreloader?'_closed':''}`}>
				{(isShowenMore?articles:showLessArray).map((article) => {
					return <Article key={generateKey(article)} q={q} isLoggedIn={isLoggedIn} toggleSave={onArticleSave} article={article} />;
				})}
			</ul>
